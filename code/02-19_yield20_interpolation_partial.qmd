...
yield20_c <- read_sf("../output/yield20_clean.geojson")
...
```
# Summary yield: clean vs interpolated

```{r clean yield summary}
summary_clean <- yield20_c %>% 
...
```{r exporting}
write_sf(idw_pred_v, "../data/yield20_interpolated.geojson",
         delete_dsn = T)
```
