...
yield19_c <- read_sf("../output/yield19_clean.geojson")
...
```
# Statistical summary: cleaned yield v/s interpolated

```{r clean yield summary}
summary_clean <- yield19_c %>% 
  summarize(n = length(yield_lbac),
            min = min(yield_lbac),
            mean = mean(yield_lbac),
            median = median(yield_lbac),
            max = max(yield_lbac),
            sd = sd(yield_lbac),
            cv = (sd/mean)*100
            ) %>% 
...
```{r interpolated yield summary}
summary_ip <- idw_pred_v %>% 
  summarize(n = length(ipyield_lbac),
            min = min(ipyield_lbac),
            mean = mean(ipyield_lbac),
            median = median(ipyield_lbac),
            max = max(ipyield_lbac),
            sd = sd(ipyield_lbac),
            cv = (sd/mean)*100
            ) %>% 
  mutate(data = 'interpolated') %>% 
  dplyr::select(data, everything())
...
```{r exporting}
write_sf(idw_pred_v, "../data/yield19_interpolated.geojson",
         delete_dsn = T)
```
